...
public class SendChatInviteServlet extends HttpServlet {
  ...
  private void sendInvite(HttpServletRequest req, HttpServletResponse resp)
    ...
    String invitee = req.getParameter("chatId");
    try {
      xmpp.sendInvitation(new JID(invitee));
      resp.getWriter().print("Chat invite sent to: "+invitee);
    } catch (Exception exception) {
      resp.getWriter().print("Unable to send chat invite to " + invitee);
    }
  }
}
