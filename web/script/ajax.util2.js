...
var populateList=function(entity, filter){
	...
	var successFn=function(resp){
		...
		if(data.length > 0){
			for (var i=0;i<data.length;i++){
				//creating a row
//				htm+='<ul id="'+entity+'-listview" data-role="listview" data-theme="g">';
				switch(entity)
				{
				case ENTITY_PRODUCT:
...
//			htm+="</ul>";
			...
			htm+='<li>No records found</li>';
		}
		$('#'+entity+'-listview').empty();
		$('#'+entity+'-listview').append(htm);
		$('#'+entity+'-listview').listview("refresh");
	}
